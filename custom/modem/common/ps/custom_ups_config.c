...
kal_uint8 mbim_imei_bcd[8] = 
    {0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA};

kal_uint8* mbim_custom_getMacAddress(void)
{
    static kal_uint8 mac_addr[] =
        {VENDOR_MAC_ADDRESS_PREFIX, 0xAA, 0xAA, 0xAA};

    kal_mem_cpy((void*)&(mac_addr[3]), (void*)&(mbim_imei_bcd[4]), 3);

    ASSERT((sizeof(mac_addr)/sizeof(kal_uint8)) == 6);
    return (kal_uint8*)mac_addr;
}

kal_uint8* ecm_custom_getGWMacAddress(void)
{
    static const kal_uint8 GWMacAddr[] =
        {VENDOR_MAC_ADDRESS_PREFIX, 0xFF, 0xFF, 0xFF};

    ASSERT((sizeof(GWMacAddr)/sizeof(kal_uint8)) == 6);
    return (kal_uint8*)GWMacAddr;
}

kal_uint16* ecm_custom_getMPMacAddress(void)
{
    // For retrieving MAC address by host
    // Length: 0x1A , Type: 0x03
    static const kal_uint16 MPMacAddr[] =
        {0x031A, VENDOR_ECM_MP_MAC_ADDRESS_FOR_USB};

    ASSERT(sizeof(MPMacAddr) == 26);
    return (kal_uint16 *)MPMacAddr;
}
#endif
