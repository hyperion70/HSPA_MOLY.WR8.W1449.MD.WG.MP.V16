...
/* KeTing: Originally, NU_DEBUG controlled whether to expose the internal data
           structures like TC_TCB. But it was not the case in 2.1.
           To sync the behavior between these two versions, we decided to
           remove the dependency of NU_DEBUG and let the internal structures
           always exposed. */

/* KeTing: Note that NU_DEBUG was also used outside Nucleus Plus, so we cannot
           just define and undefine it here.
           Instead, I used another macro NU_DEBUG_INTERNAL to do the job. */
#define NU_DEBUG_INTERNAL

//#ifdef          NU_DEBUG
#ifdef NU_DEBUG_INTERNAL
#include        "cs_defs.h"
#include        "tm_defs.h"
#include        "tc_defs.h"
#include        "mb_defs.h"
