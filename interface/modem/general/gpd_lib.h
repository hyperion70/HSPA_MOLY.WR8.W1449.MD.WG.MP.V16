...
#define GPD_LIB_CCCI_DATA_HEADER    (sizeof(CCCI_BUFF_T))
#else
#define GPD_LIB_GPD_RX_HEADER_SIZE  (0)
#define GPD_LIB_GPD_RX_FOOTER_SIZE  (CCCI_UART_GIO_PKT_FOOTER_SIZE())
#endif
...
typedef struct _bd_node
{
    ...
    union
    {
        kal_uint32*         next_bd_addr;
        struct _bd_node*    next_bd_node;
    } w1;
    union
    {
        kal_uint8*  data_buff_addr;
    } w2;
    struct
    {
        kal_uint32  data_buff_size:16;
...
bd_node* gpd_lib_umts_getFirstBD( gpd_node* node );
bd_node* gpd_lib_umts_nextBD( bd_node* bd );
kal_uint8* gpd_lib_umts_getBDDataPtr( bd_node* bd );
kal_uint16 gpd_lib_umts_getBDDataLen( bd_node* bd );
