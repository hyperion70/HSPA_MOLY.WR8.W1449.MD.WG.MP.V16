...
extern kal_uint32 SIM_drv_ts_INT, SIM_drv_ts_GPT, SIM_drv_ts_ASSERT, SIM_drv_ts_deact;
...
static void usim_deactivation(sim_HW_cb *hw_cb)
{
	...
	if(SIM_Reg(SIM0_BASE_ADDR_MTK + SIM_CTRL_MTK)&SIM_CTRL_SIMON)
	{
      ...
      /* disable any TOUT timer */
      usim_set_timeout(0, hw_cb);
		/* disable any GPT timer */
		DRV_ICC_GPTI_StopItem(usim_dcb->gpt_handle);	
		SIM_drv_ts_deact = drv_get_current_time();
...
//		PMIC_DRV_SetGpioData(0x0094,0x4000,0x4000,0x0);
	...
	{			
	   ...
	   {
	      /* I think this issue may be TOUT or GPT timeout that occur before POWER_OFF, 
	         so we did not receive POWER_OFF INT event.
	         I just disable TOUT_EN before set POWER_OFF enable */
	   	SIM_drv_ts_ASSERT = drv_get_current_time();
	   	...
	   	SIM_WriteReg((SIM0_BASE_ADDR_MTK + SIM_ATIME_MTK), (SIM_Reg(SIM0_BASE_ADDR_MTK + SIM_ATIME_MTK) - 5));
   		drv_trace8(TRACE_GROUP_4, SIM_GEMINI_GEN1, FILE_SWITCHCONTROL1, __LINE__,
				SIM_drv_ts_INT, SIM_drv_ts_GPT, SIM_drv_ts_ASSERT, SIM_Reg(SIM0_BASE_ADDR_MTK + SIM_CONF_MTK),
				SIM_Reg(SIM0_BASE_ADDR_MTK + SIM_CTRL_MTK), SIM_Reg(SIM0_BASE_ADDR_MTK + SIM_ATIME_MTK)
			);
   		drv_trace8(TRACE_GROUP_4, SIM_GEMINI_GEN1, FILE_SWITCHCONTROL1, __LINE__,
				SIM_drv_ts_deact, 0, 0, 0, 0, 0);

			/* used to check past INT event */
   		drv_trace8(TRACE_GROUP_4, SIM_GEMINI_GEN1, FILE_SWITCHCONTROL1, __LINE__,
				usim_dcb->intsta[0], usim_dcb->intsta[1], usim_dcb->intsta[2], usim_dcb->intsta[3],
				usim_dcb->intsta[4], usim_dcb->intsta[5]);
   		drv_trace8(TRACE_GROUP_4, SIM_GEMINI_GEN1, FILE_SWITCHCONTROL1, __LINE__,
				usim_dcb->intsta[6], usim_dcb->intsta[7], usim_dcb->intsta[8], usim_dcb->intsta[9],
				usim_dcb->intcnt, usim_dcb->ev_status);
		kal_sleep_task(10); //sleep a while so that message can be printed.
		}
//	   else
//	   	ASSERT(0);
	}
