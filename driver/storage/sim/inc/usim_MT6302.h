 ...
 * Description:
 * ------------
 *   Header file of USIM driver on MT6302 solution
 *
 * Author:
 * -------
 * -------
...
#ifndef _USIM_MT6302_H_
#define _USIM_MT6302_H_
#ifdef __USIM_DRV__
...
/*
	definitions start from 2 SIM interface
*/

#define GET_USIM_CB(a) &usim_cb[a]
...
usim_status_enum L1sim_Reset_All(sim_power_enum ExpectVolt, sim_power_enum *ResultVolt, kal_bool warm, kal_uint32 simInterface);
void L1sim_Enable_Enhanced_Speed_All(kal_bool enable, kal_uint32 simInterface);
kal_bool L1sim_Set_ClockStopMode_All(sim_clock_stop_enum mode, kal_uint32 simInterface);
void L1sim_PowerOff_All(kal_uint32 simInterface);
void L1sim_Get_Card_Info_All(sim_info_struct *info, kal_uint32 simInterface);
sim_status L1sim_Cmd_All(kal_uint8  *txData,kal_uint32  *txSize,kal_uint8  *rxData, kal_uint32  *rxSize, kal_uint32 simInterface);


void usim_gpt_timeout_handler(void *parameter);
void L1sim_Select_SIM_PLUS(kal_bool isSIMPLUS);
void L1sim_Select_Prefer_PhyLayer_All(sim_protocol_phy_enum T, kal_uint32 simInterface);
