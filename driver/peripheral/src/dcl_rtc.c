...
DCL_STATUS DclRTC_Control(DCL_HANDLE handle, DCL_CTRL_CMD cmd, DCL_CTRL_DATA_T *data)
{
	...
	switch (cmd)
	{
        ...
        case RTC_CMD_WRITE_OSC32CON_REG:
        {
			RTC_CTRL_WRITE_OSC32CON_REG_T *prWriteOSC32CONReg;
			prWriteOSC32CONReg = &(data->rWriteOSC32CONReg);            
            RTC_write_xosc_reg(prWriteOSC32CONReg->OSC32CON_Reg);
            return_status = STATUS_OK;
        }
        break;
