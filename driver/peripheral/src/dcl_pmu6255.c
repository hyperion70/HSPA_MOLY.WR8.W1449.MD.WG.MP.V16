...
{          
	return pmu_set_register_value(PMU_OC_CON0, flagname, val);
} 

DCL_UINT16 pmu_get_pmic_oc(PMU_FLAGS_LIST_ENUM flagname)
{
	return pmu_get_register_value(PMU_OC_CON0, flagname);
}
...
{
	PMU_VOLT_01_500000_V,   PMU_VOLT_01_800000_V,   PMU_VOLT_02_500000_V,   PMU_VOLT_02_900000_V,
    PMU_VOLT_03_000000_V,   PMU_VOLT_03_100000_V,   PMU_VOLT_03_300000_V    
};

const DCL_INT32 vsf_cal[]=
{
...
{
	PMU_SPK_VOL_06_00_dB,	PMU_SPK_VOL_07_00_dB,	PMU_SPK_VOL_08_00_dB,	PMU_SPK_VOL_09_00_dB
};
...
const PMU_PARAMETER_TABLE_ENTRY pmu_parameter_table[]=
{
	...
	{ENC(LDO_SET_CAL, VSF),			        	    vsf_cal,	        NULL,				GETARRNUM(vsf_cal)	        },
	//BUCK group
...
DCL_UINT8 pmu6255_ovp_val = 0;

DCL_UINT8 pmu6255_ldo_cal(PMU_LDO_BUCK_LIST_ENUM ldo, DCL_UINT32 vol);
void pmu6255_spk_init(PMU_CTRL_SPK_MODE_ENUM mode);

void pmu6255_spk_enable(DCL_BOOL enable);

void pmu6255_bl_init(void);

void pmu6255_bl_enable(DCL_BOOL enable);

DCL_BOOL pmu6255_cust_isink_en[PMU_ISINK_MAX];

DCL_UINT32 bl_support_level;
DCL_BOOL bl_use_pwm;

DCL_STATUS dcl_pmu6255_control_handler(DCL_HANDLE handle, DCL_CTRL_CMD cmd, DCL_CTRL_DATA_T *data)
{
	DCL_UINT16 val;
	...
	{	
		...
		switch(cmd)
		{
			...
			case LDO_BUCK_SET_VOLTAGE:
			{
				...
				pmu6255_ldo_cal(pLdoBuckCtrl->mod, pLdoBuckCtrl->voltage);
			}	
			break;
			...
			case LDO_BUCK_SET_VOLTAGE_EN: // Combination Command
			{
				...
				dcl_pmu6255_control_handler(handle, LDO_BUCK_SET_VOLTAGE, (DCL_CTRL_DATA_T *)&ldoBuckVoltage);
				dcl_pmu6255_control_handler(handle, LDO_BUCK_SET_EN, (DCL_CTRL_DATA_T *)&ldoBuckEn);
			}					
			break;
	...
	switch(cmd)
	{
		case SPK_SET_EN:
		{
			...
			{
				pmu6255_spk_enable(pSpkCtrl->enable);
			}
		...
		case SPK_SET_MODE: 
		{
		    ...
		    pmu_set_spk(SPKMODE, pSpkCtrl->mode);		    
          	pmu6255_spk_init(pSpkCtrl->mode);	
		}			
		break;
		...
		*/
		...
		case BL_SET_EN:
		{
			...
			pmu6255_bl_enable(pBlCtrl->enable);
		}
		break;
		...
		case ISINK_SET_EN:
		{
			...
			if (pIsinkCtrl->isink < PMU_ISINK_MAX)
				pmu6255_cust_isink_en[pIsinkCtrl->isink] = pIsinkCtrl->enable;
		}
		break;
		...
		case CHR_SET_CHR_EN:
		{
			...
			{
				pmu_set_chr(CSDAC_MODE, DCL_TRUE);
			}			
		}
		break;        
		...
		{
            // MT6255 Use H/W Auto Trim, If EFUSE_DATA[63] = 1'b then use EFUSE Result else use register value
            /*
...
void pmu6255_efuse_calibration(void)
{
	// E-FUSE calibration
	DCL_UINT32 val32;
	DCL_UINT16 val16;
	...
	*/
	...
	if (val32 & 0x00040000) // Bit18: Trim enable bit
	{
		...
		pmu6255_ovp_val = (DCL_UINT8)( (val32 & 0x0000C000) >> 14); // Bit[15..14]
		PMU_DRV_SetData16((MIXED_base + 0x0a14), 0x0030, pmu6255_ovp_val<<4);  // apply to VBAT_OV_VTH[1:0] (CHR_CON5[5:4])
	}

}


extern void pmu6255_customization_init(void);
...
extern void dcl_pmu6255_internal_init(void);

void dcl_pmu6255_init(void)
{

	pmu_control_handler = dcl_pmu6255_control_handler;
	...
	// pmu6255_efuse_calibration();

    dcl_pmu6255_internal_init();

	pmu6255_customization_init();
