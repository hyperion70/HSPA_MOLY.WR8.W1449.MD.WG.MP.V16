...
//                Exported APIs                 //
//////////////////////////////////////////////////
extern kal_bool pmic6350_reg_write(kal_uint16 reg, kal_uint16 val);
extern kal_bool pmic6350_reg_read(kal_uint16 reg, kal_uint16 *pVal);
...
kal_uint8 pmic6350_eco_version;
kal_uint16 pmic6350_reg[PMIC_MAX_REG_NUM];
kal_uint16 pmic6350_gpio_reg[GPIO_MAX_REG_NUM];
...
const PMIC_FLAG_TABLE_ENTRY pmic_flags_table[] = 
{
    ...
    {AUXADC_CON20,		RG_MD_RQST_MASK,		RG_MD_RQST_SHIFT,		},
	 ...
	 {VPA_CON9, 	      RG_VPA_SW_VOSEL_MASK,	  RG_VPA_SW_VOSEL_SHIFT, 	  },
	 {VPA_CON10, 	      RG_VPA_SW_VOSEL_ON_MASK,	  RG_VPA_SW_VOSEL_ON_SHIFT, 	  },
...
// Write Whole Bytes
void dcl_pmic6350_byte_write(DCL_UINT16 addr, DCL_UINT16 val)
{
...
void dcl_pmic6350_field_write(PMIC_FLAGS_LIST_ENUM flag, DCL_UINT16 sel)
{
    ...
    pmic_reg_log.reg_before_write = pmic6350_reg[pTable[flag].offset];

    pmic6350_reg[pTable[flag].offset] &= ~(pTable[flag].mask << pTable[flag].shift);
    pmic6350_reg[pTable[flag].offset] |= ((kal_uint16)sel << pTable[flag].shift);    

#if defined(__MD1__)    
    DrvPWRAP_WACS0(PMIC_WRAP_WRITE, PMIC_BASE + pTable[flag].offset, pmic6350_reg[pTable[flag].offset], 0x00);
#else
    DrvPWRAP_WACS1(PMIC_WRAP_WRITE, PMIC_BASE + pTable[flag].offset, pmic6350_reg[pTable[flag].offset], 0x00);
#endif
    ...
    pmic_reg_log.reg_data = pmic6350_reg[pTable[flag].offset];
...
void dcl_pmic6350_gpio_field_write(PMIC_FLAGS_LIST_ENUM flag, DCL_UINT16 sel)
{
    const PMIC_FLAG_TABLE_ENTRY *pTable = pmic_flags_table;
    kal_uint32 savedMask = 0;
#if !defined(__UBL__) && !defined(__FUE__)
    ...
    pmic_reg_log.command_flag = flag;
    pmic_reg_log.reg_before_write = pmic6350_gpio_reg[pTable[flag].offset];
    
#if defined(DCL_PMIC_ACCESS_TIME_LOG)
    pmic_access_duration_log[PMIC_INTERFACE_DBG][pmic_access_duration_index].start_time = ust_get_current_time();
#endif

    pmic6350_gpio_reg[pTable[flag].offset] &= ~(pTable[flag].mask << pTable[flag].shift);
    pmic6350_gpio_reg[pTable[flag].offset] |= ((kal_uint16)sel << pTable[flag].shift);    

#if defined(__MD1__)
    DrvPWRAP_WACS0(PMIC_WRAP_WRITE, GPIO_BASE + pTable[flag].offset, pmic6350_gpio_reg[pTable[flag].offset], 0x00);
#else
    DrvPWRAP_WACS1(PMIC_WRAP_WRITE, GPIO_BASE + pTable[flag].offset, pmic6350_gpio_reg[pTable[flag].offset], 0x00);
#endif
    ...
    pmic_reg_log.reg_data = pmic6350_gpio_reg[pTable[flag].offset];
...
// Read Whole Bytes
DCL_UINT16 dcl_pmic6350_byte_return(DCL_UINT16 addr)
{
    DCL_UINT16 reg_temp;
    kal_uint32 savedMask = 0;    
...
// Read register field
DCL_UINT16 dcl_pmic6350_field_read(PMIC_FLAGS_LIST_ENUM flag)
{
    const PMIC_FLAG_TABLE_ENTRY *pTable = pmic_flags_table;
    kal_uint32 savedMask = 0;
...
#if defined(__MD1__)
    DrvPWRAP_WACS0(PMIC_WRAP_READ, PMIC_BASE + pTable[flag].offset, 0x00, &pmic6350_reg[pTable[flag].offset]);
#else
    DrvPWRAP_WACS1(PMIC_WRAP_READ, PMIC_BASE + pTable[flag].offset, 0x00, &pmic6350_reg[pTable[flag].offset]);
#endif

    reg_return = ((pmic6350_reg[pTable[flag].offset] & (pTable[flag].mask << pTable[flag].shift)) >> pTable[flag].shift);
...
void pmic6350_EM_reg_write(kal_uint16 reg, kal_uint16 val){
    dcl_pmic6350_byte_write(reg, val);
}

kal_uint16 pmic6350_EM_reg_read(kal_uint16 reg){
    return  dcl_pmic6350_byte_return(reg);
}
...
/*L1_3G only use 0.5->0.6->0.7 ...etc, so unused value we set as PMU_VOLT_INVALID */
const DCL_UINT32 vpa_vosel[] =
{
...
DCL_STATUS PMIC6350_control_handler(DCL_HANDLE handle,DCL_CTRL_CMD cmd,DCL_CTRL_DATA_T *data)
{
    DCL_UINT16	regVal;
    ...
    switch(cmd)
    {
        case LDO_BUCK_SET_EN:
        {
            ...
            switch(pLdoBuckCtrl->mod)
            {
                case VRF18:
                {
                    dcl_pmic6350_field_write(RG_VRF18_EN, pLdoBuckCtrl->enable);
                ...
                case VSIM:
                {
                    dcl_pmic6350_field_write(RG_VSIM1_EN, pLdoBuckCtrl->enable);
                    return_val = STATUS_OK;
                }
                break;
                ...
                case VSIM2:
                {
                    dcl_pmic6350_field_write(RG_VSIM2_EN, pLdoBuckCtrl->enable);
                    return_val = STATUS_OK;
                }
                break;
                ...
                case VPA_SW:
                {
                    dcl_pmic6350_field_write(RG_VPA_EN, pLdoBuckCtrl->enable);
                    return_val = STATUS_OK;
                }
                break;
        ...
        case LDO_BUCK_SET_LP_MODE_SET:
        {
            ...
            {               
                case VRF18:
                {
                    dcl_pmic6350_field_write(VRF18_LP_MODE_SET, pLdoBuckCtrl->enable);
                    return_val = STATUS_OK;
                }
                break;
                ...
                case VSIM1:
                {
                    dcl_pmic6350_field_write(VSIM1_LP_MODE_SET, pLdoBuckCtrl->enable);
                    return_val = STATUS_OK;
                }
                break;
                ...
                case VSIM2:
                {
                    dcl_pmic6350_field_write(VSIM2_LP_MODE_SET, pLdoBuckCtrl->enable);
                    return_val = STATUS_OK;
                }
                break;                
        ...
        case LDO_BUCK_SET_LP_SEL:
        {
            ...
            {               
                case VRF18:
                {
                    dcl_pmic6350_field_write(VRF18_LP_MODE_SET, pLdoBuckCtrl->onSel);
                    return_val = STATUS_OK;
                }
                break;
                ...
                case VSIM1:
                {
                    dcl_pmic6350_field_write(VSIM1_LP_MODE_SET, pLdoBuckCtrl->onSel);
                    return_val = STATUS_OK;
                }
                break;
                ...
                case VSIM2:
                {
                    dcl_pmic6350_field_write(VSIM2_LP_MODE_SET, pLdoBuckCtrl->onSel);
                    return_val = STATUS_OK;
                }
                break;                
        ...
        case LDO_BUCK_SET_VOLTAGE:
        {
            ...
            {               
                case VSIM1:
                {
                    dcl_pmic6350_field_write(RG_VSIM1_VOSEL, regVal);
                    return_val = STATUS_OK;
                }
                break;
                ...
                case VSIM2:
                {
                    dcl_pmic6350_field_write(RG_VSIM2_VOSEL, regVal);
                    return_val = STATUS_OK;
                }
                break;
                ...
                case VPA_SW:
                {
                    dcl_pmic6350_field_write(RG_VPA_SW_VOSEL, regVal);
                    return_val = STATUS_OK;
                }
                break;
		...
		case LDO_BUCK_SET_ON_CTRL:
		{
            ...
            switch(pLdoBuckCtrl->mod)
            {
                ...
                {
                    // 0: SW control, 1: HW
                    dcl_pmic6350_field_write(VRF18_ON_CTRL, pLdoBuckCtrl->mode);                    
                    return_val = STATUS_OK;
                }
                break;
        ...
        case VPA_SET_EN:
        {
            ...
            dcl_pmic6350_field_write(VPA_EN, pVpaSetEn->enable);
            return_val = STATUS_OK;
        }
        break;
        ...
        case VPA_SET_FPWM:
        {
            ...
            dcl_pmic6350_field_write(RG_VPA_MODESET, pVpaSetFpwm->enable);
            return_val = STATUS_OK;
        }
        break;             
        ...
        case SIM_SET_STB_SIO_MODE:
        {
			...
			switch(pSimCtrl->vsimIdx)
			{
				case PMIC_VSIM1:
				{
                    dcl_pmic6350_field_write(RG_STB_SIM1_SIO, pSimCtrl->mode);
                    return_val = STATUS_OK;
				}
				break;	
				...
				case PMIC_VSIM2:
				{
					dcl_pmic6350_field_write(RG_STB_SIM2_SIO, pSimCtrl->mode);
                    return_val = STATUS_OK;
				}
				break;					
        ...
        {   
			...
			switch(pSimrstCtrl->vsimIdx)
			{
				case PMIC_VSIM1:
				{
                    dcl_pmic6350_gpio_field_write(GPIO15_DOUT_SET, DCL_TRUE);
                    return_val = STATUS_OK;
				}
				break;	
				...
				case PMIC_VSIM2:
				{
					dcl_pmic6350_gpio_field_write(GPIO17_DOUT_SET, DCL_TRUE);
                    return_val = STATUS_OK;
				}
				break;					
        ...
        {   
			...
			switch(pSimrstCtrl->vsimIdx)
			{
				case PMIC_VSIM1:
				{
                    dcl_pmic6350_gpio_field_write(GPIO15_DOUT_CLR, DCL_TRUE);
                    return_val = STATUS_OK;
				}
				break;	
				...
				case PMIC_VSIM2:
				{
					dcl_pmic6350_gpio_field_write(GPIO17_DOUT_CLR, DCL_TRUE);
                    return_val = STATUS_OK;
				}
				break;					
        ...
        case ADC_SET_RQST:
        {
            ...
            dcl_pmic6350_byte_write(TOP_CKPDN1_SET, (0x1 << RG_CLKSQ_EN_AUX_MD_SHIFT));
            dcl_pmic6350_byte_write(AUXADC_CON27, (0x0 << 15));
            dcl_pmic6350_field_write(RG_MD_RQST, 0x0); // Need Set 0 first
			
            dcl_pmic6350_field_write(RG_MD_RQST, pAdcCtrl->enable);
...
#if defined(FPGA_CTP)
            dbg_print("[PMIC_WRAP_MD] ADC Data Status = %x\n", dcl_pmic6350_field_read(RG_ADC_RDY_MD));
#endif
            pAdcCtrl->status = (DCL_BOOL)dcl_pmic6350_field_read(RG_ADC_RDY_MD);
        ...
        case ADC_GET_OUT_MD:
        {
            ...
            pAdcCtrl->data = (DCL_UINT32)dcl_pmic6350_field_read(RG_ADC_OUT_MD); 
            AUXADC_Status = AUXADC_READ_DATA;
            dcl_pmic6350_field_write(RG_MD_RQST, 0x0);
            dcl_pmic6350_byte_write(TOP_CKPDN1_CLR, (0x1 << RG_CLKSQ_EN_AUX_MD_SHIFT));
            dcl_pmic6350_byte_write(AUXADC_CON27, (0x1 << 15));
        ...
        {			
            ...
            pmic6350_EM_reg_write(pChrCtrl->offset, pChrCtrl->value);
            return_val = STATUS_OK;
        }
        break;
        ...
        case MISC_GET_REGISTER_VALUE:
        {
            ...
            pChrCtrl->value = pmic6350_EM_reg_read(pChrCtrl->offset);
            return_val = STATUS_OK;
        }
        break;
...
extern void dcl_pmic6350_internal_init(void);
...
void dcl_pmic6350_init(void){
    kal_uint32 i;

	pmu_control_handler = PMIC6350_control_handler;
    ...
    DrvPWRAP_Init();
#endif    
    pmic6350_eco_version = PMIC6350_ECO_1_VERSION;

    // Get PMIC6350 ECO version
    {
        kal_uint16 eco_version = 0;
        eco_version = dcl_pmic6350_byte_return(CID);       

        if (eco_version == MT6350_E1_CID)
        {
            pmic6350_eco_version = PMIC6350_ECO_1_VERSION;
        }
        else
        {
            pmic6350_eco_version = PMIC6350_ECO_2_VERSION;
        }
        
    }

    for (i = 0; i < PMIC_MAX_REG_NUM; i += 2){
        pmic6350_reg[i] = dcl_pmic6350_byte_return(i); 
    }

#if !defined(__SMART_PHONE_MODEM__)    
    dcl_pmic6350_modem_init();
#endif
    dcl_pmic6350_internal_init();

  //  pmic6350_customization_init();
#if defined(PMIC_UNIT_TEST)
    PMIC_Read_All();
    PMIC_Unit_Test();
    PMIC_Read_All();
...
void dcl_pmic6350_modem_init(void)
{
}

DCL_UINT32 DRV_Read_PMIC_Data(DCL_UINT32 pmic_addr)
{
    return dcl_pmic6350_byte_return(pmic_addr);
}

void DRV_Write_PMIC_Data(DCL_UINT32 pmic_addr, DCL_UINT32 value)
{
    dcl_pmic6350_byte_write(pmic_addr, value);
}
...
#endif //#if (defined(PMIC_6350_REG_API))
