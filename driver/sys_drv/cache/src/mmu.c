...
kal_uint32 MMU_Init(void)
{
   ...
   do {
      ...
      #if defined(__ARM9_MMU__)
      /* configure non-cached non-buffered RAM as READ_WRITE and non-cached non-buffered region */
      custom_query_noncached_extsram_rw_ncnb_region(&regionp);
      while (ret == 0 && (regionp->addr != 0 || regionp->len != 0)) {
         if (regionp->len == 0) {
            regionp++;
            continue;
         ...
         ret = dynamic_switch_mmu_permission_cb(regionp->addr, len, ((MMU_PAGE_CONFIG_PTR)&pagc)->permission, NONCACHEABLE_NONBUFFERABLE);
         regionp++;
      }
...
#if defined(IC_BURNIN_TEST) || defined(IC_MODULE_TEST) || defined(__SSDVT__)
...
#define X_REGISTER_REGION_NCNB_CONST(a,b,c,d,e)   if(e==0)\
                                                     mk_pt(((b)>>20)<<20, ((c)>>20)<<20, (((d)+(1<<20)-1)>>20)<<20, (MMU_PAGE_CONFIG_PTR)&pagc, DEF_PAGE_TABLE);\
                                                  else\
                                                     mk_pt(((b)>>20)<<20, ((c)>>20)<<20, (((d)+(1<<20)-1)>>20)<<20, (MMU_PAGE_CONFIG_PTR)&pagc2, DEF_PAGE_TABLE);
#define X_REGISTER_REGION_NCB_CONST(a,b,c,d,e) 
   #include "reg_region.h"
#undef X_REGISTER_REGION_NCNB_CONST
#undef X_REGISTER_REGION_NCB_CONST
