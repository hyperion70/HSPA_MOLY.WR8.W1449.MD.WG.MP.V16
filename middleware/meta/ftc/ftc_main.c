    ...
    if( (ptrMsg->src_mod_id == MOD_FT) )
    {
        ...
        switch (ptrMsg->msg_id)
        {
            case MSG_ID_FTC_BASIC_REQ:
            {
                ...
                ftc_ptr_loc_para=(ftc_basic_cnf_struct *)(ptrMsg->local_para_ptr);
                ftc_ptr_loc_para->status = FTC_CNF_OK;                
                msg_send6(MOD_FTC ,MOD_FT, FTC_FT_SAP, MSG_ID_FTC_BASIC_CNF,ptrMsg->local_para_ptr,peer_buff_new);
                break;
            }
            default:
                break;
