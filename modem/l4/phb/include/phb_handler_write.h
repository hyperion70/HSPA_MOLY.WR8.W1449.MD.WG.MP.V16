...
//#error "stack_ltlcom.h should be included"
    ...
    typedef enum
    {
        PHB_WRITE_STAGE_NONE,
        PHB_WRITE_STAGE_CONTINUE = 0x03,   /* To avoid conflict with READ_CONTINUE */
    #ifdef __PHB_USIM_SYNC__        
        PHB_WRITE_STAGE_UPDATE_UID,
        PHB_WRITE_STAGE_UPDATE_PUID,
        PHB_WRITE_STAGE_UPDATE_CC,
        PHB_WRITE_STAGE_UPDATE_PSC,        
    #endif
        PHB_WRITE_STAGE_DONE,
        PHB_WRITE_STAGE_TOTAL
    } phb_write_stage_enum;
    ...
    extern void phb_check_write_usim_handler(ilm_struct *ilm_ptr);
    extern void phb_write_usim_handler(ilm_struct *ilm_ptr);
