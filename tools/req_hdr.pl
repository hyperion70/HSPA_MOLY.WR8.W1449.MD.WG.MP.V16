...
#
#!/usr/bin/perl
# find out required .(hpp|h|dat|c|hex|incl|inc|msg) for the header sources listed in .lis and in .dep
...
foreach (@reading)
{
   if ((s/\S+\\(\w+)\.cp?p?\s*$/\L$1.obj/i) || (s/\S+\\(\w+)\.s\s*$/\L$1.obj/i))# extract '*.c/cpp/s'
   {
      $obj_list .= " $_";
#      print "$_\n";
      $obj_count++;
...
#         print "\n\n$dep_count\t";
         while ($reading =~ /\b\S+\.(hpp|hex|dat|c|h|incl|inc|msg)/)      # find out next .(hpp|h|dat|c|hex) in this line
         {
            $hdr_name = "\L$&";
            $reading = ($`).($');
...
#                  print "[$hdr_name]\t";
   ...
   elsif (/\.(hpp|hex|dat|c|h|incl|inc|msg)/ && $needed_obj == 1)
   {
      $reading = lc($_);
