...
sub GenSECTIONS
{
    ...
    foreach my $item (@$RegionList_ref)
    {
        ...
        {
            my @temp = map{ $_ if($_ ne $strType) } @$item;
...
sub GenInputSectionsByAttribute  # used by GenRORWInputSections and GenZIInputSections 
{
    ...
    if(scalar(@FileList)>0)
    {
        ...
        foreach my $file (@FileList)
        {
            ...
            $strChipDependentPath = $InputSectionFolder.$file if($file =~/$strChip/);
            $strCommonPath = $InputSectionFolder.$file if($file =~/$Common/);
            $strDefaultChipPath = $InputSectionFolder.$file if($file =~/$ChipDefault/);
            $strHeadPath = $InputSectionFolder.$file if($file =~/$Head/);
        }
        if(-e $strHeadPath)
        {
